...
for i in range(3):  # x, y, z translation
    move_dir = np.zeros(6)
    for j in range(NO_QUANTIZATION):
        dp = (TRANSLATION_DISPLACEMENT_RANGE[1] -
              TRANSLATION_DISPLACEMENT_RANGE[0]) / NO_QUANTIZATION
...
for i in range(3):  # x, y, z rotation
    move_dir = np.zeros(6)
    for j in range(NO_QUANTIZATION):
        dp = (ROTATION_DISPLACEMENT_RANGE[1] -
              ROTATION_DISPLACEMENT_RANGE[0]) / NO_QUANTIZATION
...
for i in range(2):  # x, y translation
    ...
    for j in range(NO_QUANTIZATION):
        for k in range(NO_QUANTIZATION):
            dv = (
                SLIDING_SPEED_FACTOR_RANGE[1] - SLIDING_SPEED_FACTOR_RANGE[0]) / NO_QUANTIZATION
            v = SLIDING_SPEED_FACTOR_RANGE[0] + dv / 2 + j * dv

            dfs = (FORCE_THRESH_RANGE[1] -
                   FORCE_THRESH_RANGE[0]) / NO_QUANTIZATION
...
for i in range(3):  # x, y, z rotation
    ...
    for j in range(NO_QUANTIZATION):
        for k in range(NO_QUANTIZATION):
            dv = (SLIDING_SPEED_FACTOR_RANGE[1] -
                  SLIDING_SPEED_FACTOR_RANGE[0]) / NO_QUANTIZATION
            v = SLIDING_SPEED_FACTOR_RANGE[0] + dv / 2 + j * dv

            dfs = (TORQUE_THRESH_RANGE[1] -
                   TORQUE_THRESH_RANGE[0]) / NO_QUANTIZATION
...
for j in range(NO_QUANTIZATION):
    for k in range(NO_QUANTIZATION):
        dkd = (KD_ADMITTANCE_ROT_RANGE[1] -
               KD_ADMITTANCE_ROT_RANGE[0]) / NO_QUANTIZATION
        kd = KD_ADMITTANCE_ROT_RANGE[0] + dkd / 2 + j * dkd

        df = (INSERTION_FORCE_RANGE[1] -
              INSERTION_FORCE_RANGE[0]) / NO_QUANTIZATION
...
contact_action_idx = list(
    range(no_free_actions, no_free_actions + no_contact_actions))
...
sampler_config = {
    ...
    "sampler_kwargs": {
        ...
        "batch_B": 4,  # no environments, this will be divided equally to no.
                       # parallel envs
