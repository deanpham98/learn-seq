...
SIM_RUN_NAME = "run_round_10_02_14_15"
REAL_EXP_NAME = "real-prim-rew-1-10"
REAL_RUN_NAME = "checkpoint-round"
...
def compare_envs(sim_config, real_config):
    ...
    real_hole_pos = real_config.env_config["hole_pos"]
    real_hole_quat = real_config.env_config["hole_quat"]
    ...
    real_env_config["wrapper_kwargs"]["hole_pos_error_range"] = (np.zeros(3), np.zeros(3))
    real_env_config["wrapper_kwargs"]["hole_rot_error_range"] = (np.zeros(3), np.zeros(3))
    # real_env = gym_make(**real_env_config)

    return sim_env, sim_env

def evaluate():
    initial_pos = np.array([0, 0, 0.01])
    ...
    sim_agent = agent_class(initial_model_state_dict=state_dict,
    ...
    real_agent = agent_class(initial_model_state_dict=state_dict,
                        model_kwargs=model_kwargs)
    ...
    for i in range(3):
        for j in range(2):
            ax[i, j].plot(len(sim_obs_seq), np.array(sim_obs_seq)[:, i+3*j])
            ax[i, j].plot(len(real_obs_seq), np.array(real_obs_seq)[:, i+3*j])
